...
class DateSicknessBeganControllerSpec extends SpecBase with MockitoSugar {
  ...
  def getRequest(): FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest(GET, dateSicknessBeganRoute)

  def postRequest(): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, dateSicknessBeganRoute)
  ...
  "DateSicknessBegan Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[DateSicknessBeganView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(DateSicknessBeganPage, validAnswer).success.value
